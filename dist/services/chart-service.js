...
class ChartService {
    static GenerateCandleChartPost(symbol, timeframe, candleInfo, ExchangeName) {
        return new Promise((resolve, reject) => {
            ...
            request({
                ...
                resolve("http://wcsera.com/" + response.body);
            });
        });
    }
    static GenerateCapitalChart(CapitalData) {
        return new Promise((resolve, reject) => {
            const marketCap = CapitalData;
            var exportSettings = {
                type: 'png',
                resources: {
                    files: 'http://code.highcharts.com/themes/dark-unica.js',
                ...
                options: {
                    title: {
                        text: ' Market Cap Chart (24H) - Generated by @WCSEBot',
                        style: {
                            color: "white",
                            fontSize: "14px",
                            fontWeight: "bold",
    ...
    ;
    static GenerateCapitalChartOld3(CapitalData, VolumeData) {
        return new Promise((resolve, reject) => {
            var MarketCap = [];
            var VolumeCap = [];
            var CapDate = [];
            for (let i = 0; i < CapitalData.Cap.length; i += 1) {
                MarketCap.push(parseFloat(CapitalData.Cap[i][0]), CapitalData.Cap[i][1]);
                VolumeCap.push([
                    parseFloat(CapitalData.Volume[i][0]),
                    VolumeData[i][1],
                ]);
            }
            var exportSettings = {
    ...
    static GenerateCapitalChartWorkingCap(CapitalData) {
        return new Promise((resolve, reject) => {
